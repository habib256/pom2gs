...
| Sony 3.5" drive + 800K GCR codec | `floppy.cpp` mac_floppy + KEGS `iwm.c` | `Sony35` | 🟢 status/command tables, read+write, codec = exact KEGS/ROM nibblizer port; block write/read/verify through the ROM firmware gated (`floppy_rw35_test`); FORMAT/tach-calibration untested |
